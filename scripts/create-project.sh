...
# ICM Automation - Project Creator
VERSION="1.0.0"

show_help() {
    cat << EOF
ICM Automation v${VERSION}

Usage: icm-automation create-project <PROJECT_NAME>
...
EOF
    ...
    # ✅ Get project name
    if [ -z "$1" ]; then
        echo "❌ Usage: icm-automation create-project <ProjectName>"
        exit 1
    fi

    PROJECT_NAME="$1"
    PACKAGE_PATH="com/example/automation/icm/$PROJECT_NAME"
    ROOT_DIR="$PROJECT_NAME"
    SRC_DIR="$ROOT_DIR/src/test/java/$PACKAGE_PATH"

    # 🧑‍💻 Detect system user info
    SYSTEM_USER_NAME=$(git config user.name 2>/dev/null || echo "$USER")
    SYSTEM_USER_EMAIL=$(git config user.email 2>/dev/null || echo "unknown@example.com")
    GENERATED_DATE=$(date)
 ...
 */
package com.example.automation.icm.$PROJECT_NAME;
...
case "${1}" in
    create-project)
        if [ -z "$2" ]; then
            ...
            echo "Usage: icm-automation create-project <PROJECT_NAME>"
            exit 1
        fi
        create_gradle_project "$2"
        ;;
    version)
        echo "ICM Automation v${VERSION}"
        ;;
    help|--help|-h)
        show_help
        ;;
